 ...
 *
 * Tombstones every active account leaf at or under `path_fqn` in one all-or-nothing operation. A leaf path and a group path are handled identically.
 */
export const deleteAccountsByPath = <ThrowOnError extends boolean = false>(options: Options<DeleteAccountsByPathData, ThrowOnError>): RequestResult<DeleteAccountsByPathResponses, DeleteAccountsByPathErrors, ThrowOnError> => (options.client ?? client).post<DeleteAccountsByPathResponses, DeleteAccountsByPathErrors, ThrowOnError>({
