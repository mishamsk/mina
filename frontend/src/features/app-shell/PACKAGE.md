...
- Source-seeded recurring-definition drafts are route-independent overlays; saves refresh a mounted recurring-definition list and invalidate transaction/register snapshots, the Recurring route stays inert while its global draft is open, and closure restores a visible connected opener (including template actions), monitors a pending transaction refresh for row removal, re-resolves remounted transaction actions, or falls back to a visible transaction surface or the current route heading.
