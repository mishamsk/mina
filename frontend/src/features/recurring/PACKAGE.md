...
- Entity-specific backend picker contexts keep seeded active hidden references labeled and removable while excluding them as fresh choices; returned account type and currency immediately reconcile the selected row even before broader lookups load, while the editor does not derive picker eligibility or ordering from that snapshot.
