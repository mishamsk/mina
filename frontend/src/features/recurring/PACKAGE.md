...
- Every definition mutation refreshes the `/recurring` route's sole mounted definition snapshot using its latest query when present and invalidates transaction/register views with a fresh occurrence catch-up; the shared posted confirm-next operation coalesces route-overlapping consumers and publishes its pending state through refresh, while its refresh path also updates account headers, featured balances, and Overview because it posts a transaction.
