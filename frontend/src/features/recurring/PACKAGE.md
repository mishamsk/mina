...
- The editor writes a complete balanced record set with the loaded definition ETag, sends a null anchor unless the user changed that field, blocks changed past anchors against the server accounting date when available, and maps server anchor errors to that field. A stale replacement retains the draft, adopts the winner's ETag and untouched anchor, and requires an explicit retry. Categories appear only for `flow` records; a template-seeded category on a non-flow record remains visible with its full path available as a tooltip only as a clearable invalid default. If a follow-up pause/resume request fails after the write, keep the editor open, retain the successful replacement as the retry baseline, and report the partial result.
...
- Does not own expected-transaction review or actions, which belong to Transactions; REST endpoint generation, schedule semantics, accounting validation, and lookup persistence belong elsewhere.
