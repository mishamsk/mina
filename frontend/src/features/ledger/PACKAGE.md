...
- The app-shell-owned `EntryModal` is the single create/edit/split/duplicate surface on every route; `?entry=` owns its shareable launch state, composes with page URL state, and rejects ineligible Split targets through the standard unavailable treatment.
