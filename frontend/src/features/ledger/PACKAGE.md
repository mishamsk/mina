...
- Transaction class, shapes, record roles, display titles, primary amounts, USD equivalents, and record amounts come from REST responses; shared transaction tables keep page-local native/USD browse state, preserve the bare `+` more-parts fallback, and show `N/A` for unavailable USD without deriving totals.
...
- After a successful amount save filters out its row, focus falls back in order to the same-index transaction row, an enabled pagination control or its footer, an available empty-state action, then the route-level list restore target; browser embeddings preserve the `data-transaction-row`, `transactions-pagination-footer`, and `data-transaction-detail-restore-target` hooks, while the Transactions page also preserves `data-transaction-empty-action`.
- Transaction-row lifted member display ignores unattributed records.
- The transaction detail panel is read-only: deferred initial focus yields to a newer deliberate focus target; activating its transaction row again closes it with focus restored, activating another row switches it directly, record rows only toggle inert exact-value disclosures, account display-label links navigate to account registers, editing goes through footer Edit/Duplicate/Split modal launches, and entity chips filter the underlying list.
- Transaction detail and account-register peek share the same read-only transaction-detail content component; full detail stacks native and USD/`N/A` display amounts while account-register peeks stay native-only, following the owning [Transactions specification](../../../../docs/webui-design.md#2-transactions--phase-2-core-screen).
...
- Transaction rows and detail footers share the action-applicability decisions from the owning [Transactions specification](../../../../docs/webui-design.md#2-transactions--phase-2-core-screen); rows themselves are the only detail-opening affordance.
- Transaction detail panel owns tombstone confirmation plus post/cancel/restore controls and delegates each transaction mutation to the owning page.
...
- Transaction-page request ownership survives overlay-only URL changes so opening or closing entry/detail cannot orphan an equivalent in-flight list load.
- Single-transaction Post confirms a current-local-time default that the user may edit, sends every pending record ID plus that exact instant through the atomic bulk-settlement operation, then blocks on the current page, selected detail, featured balances, and Overview refreshes while invalidating register and reference snapshots for asynchronous reload; local row or detail action state owns errors and focus recovery.
- `TransactionBrowserToolbar` owns filter-bar visibility, the page-local accessible native/USD browse toggle, and stable toolbar geometry independent of overlapping detail; pages retain URL-filter ownership and supply chip clearing that preserves standing search and class controls.
...
- Entry supports spend, income, refund, transfer, and exchange shorthand endpoints; create-only per-tab pending intent applies to every produced balance record across shorthand and composed journals, persists until save, and resets unchecked. Spend merchant rows share one draft representation, spend/income/refund balance fields accept `owned` and `party` accounts, Transfer may compose an ordinary expense charge, and Advanced previews the server's dry classification. Transaction-entry interaction follows the owning [Transaction entry specification](../../../../docs/webui-design.md#3-transaction-entry--phase-2).
- Advanced replacement keeps settlement dates collapsed by default; `Edit pending/posted dates` exposes prefilled exact timestamps for eligible balance records, preserves omitted source history, and lets the service default newly required later-change dates to operation time.
...
- The app-shell-owned `EntryModal` is the single create/edit/split/duplicate surface on every route; `?entry=` owns its shareable launch state, composes with page URL state, and rejects ineligible Split targets through the standard unavailable treatment. Local launches claim URL synchronization before rendering, and a delayed close-autofocus callback yields to any newer open lifecycle without consuming its restore target.
...
- Saved-transaction Edit/Split saves are full replacements owned by `EntryModal`; active spend/income Split appends a blank manual allocation row seeded from the first matching REST-role flow record, and successful saves fan out to displayed transaction browsers, detail, balances, overview, registers, and reference snapshots.
