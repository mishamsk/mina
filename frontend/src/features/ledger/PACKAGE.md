...
- When a successful inline save filters out its row, focus falls back in order to the same-index transaction row, an enabled pagination control or its footer, the empty-state action, then the route-level list restore target; browser embeddings preserve the `data-transaction-row`, `transactions-pagination-footer`, `data-transaction-empty-action`, and `data-transaction-detail-restore-target` hooks that define that order.
...
- Category edits target categorized `flow` records only; tags, balance-record settlement, and reconciliation use their narrow record bulk APIs, while member, memo, initiated dates, and simple row amounts use atomic transaction replacement built from the displayed transaction shape.
