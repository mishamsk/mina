...
- Recurring-definition invalidation replaces loaded page identities and marks them stale without discarding their transaction snapshots; inline snapshot updates preserve that staleness until the owning resource refreshes.
...
- Account, Category, Tag, and Member management snapshot identities and request state include normalized search, hidden visibility, and applicable typed filters; restoring a loaded key clears another key's transient request state, and request generations reject late results.
- Entity picker candidates are request-scoped backend results and are not stored in Zustand; broader lookup snapshots remain available for record rendering and draft transformations.
- Template mutation versions advance even without a complete snapshot so mounted queried consumers can reload their independent result sets.
- Account transaction detail reads and retained mutation responses seed the disposable cache; register loading never fetches transaction detail, and mutation invalidation removes affected entries.
- Transaction-entry route results apply only to their exact requested entry. An entry launch waits for Edit-mode amount saves to succeed, and cancellation or any failed save discards the deferred launch; its optional opener, create-only initiated date, and resolved initial template are transient launch state, its captured Recent context may contain complete transactions or register presentation summaries, and a shape-changing inline conflict may carry its transient amount and matching record IDs into an Advanced edit launch.
- Template editor launches are route-independent; recurring-definition launches capture their origin pathname. Both retain their opener and source defaults or edited definition only in transient memory; an open recurring editor rejects replacement launches, and closing it clears its payload. The Recurring route owns fragment resolution for restored history entries.
