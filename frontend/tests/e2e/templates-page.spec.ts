...
test("failed first template search does not show the complete tree", async ({
  page,
}) => {
  await page.goto("/templates");
  const rows = page.getByTestId("templates-tree-row");
  await expect(rows.first()).toBeVisible();
  await page.route("**/api/transaction-templates?*", async (route) => {
    const url = new URL(route.request().url());
    if (route.request().method() !== "GET" || !url.searchParams.has("q")) {
      await route.continue();
      return;
    }
    await route.fulfill({
      body: JSON.stringify({
        error: { code: "internal_error", message: "search unavailable" },
      }),
      contentType: "application/json",
      status: 500,
    });
...
test("cold template search skips the complete snapshot load", async ({
  page,
}) => {
  ...
  await page.goto("/templates?q=Missing");
  await expect(
    page.getByText("No templates match the current full-path search."),
  ).toBeVisible();
  expect(filteredRequestCount).toBe(1);
  expect(completeRequestCount).toBe(0);
});

test("filtered template rows are inert while a mutation refreshes", async ({
  page,
}, testInfo) => {
  const unique = `${testInfo.project.name.replace(/[^A-Za-z0-9]+/g, "")}${Date.now()}`;
  const fqn = `E2E:${unique}:Filtered mutation`;
  await createTemplate(page, fqn, [{ memo: unique }]);
  let releaseRefresh = () => {};
  ...
  await page.goto(`/templates?q=${encodeURIComponent(unique)}`);
  const row = page.getByTestId("templates-tree-row").filter({ hasText: fqn });
  await expect(row).toBeVisible();
  await row.getByRole("button", { name: "Delete template" }).click();
  await page
    .getByRole("alertdialog", { name: "Delete template" })
    .getByRole("button", { name: "Delete template" })
    .click();
  await refreshStarted;
