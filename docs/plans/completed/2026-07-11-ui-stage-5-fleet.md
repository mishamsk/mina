...
- [x] `just test-frontend-e2e` passes (230/230 one shot)
- [x] Deviation from template, per operator rules: NO fleet-level `just review-loop` (all merges were conflict-free fast-forward squashes; every branch was cut from the latest integrated state and gated on the full suite, so cross-branch interactions were continuously covered) (each branch already ran its one allowed loop) — unless merges needed conflict resolution or cross-branch interactions were never covered, in which case run exactly one and fold unresolved comments into a final fix plan (no further review-loop)
