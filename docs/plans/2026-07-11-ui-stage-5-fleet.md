...
- [ ] Task 17: `gwrc` — Account credit-limit UX: empty-state add button and credit-card indicator icon (after Tasks 12–14 settle the accounts tree name column and row semantics; doc amendment: indicator placement in webui-design) — branch `gwrc-credit-limit-ux`
- [ ] Task 18: `dcjx` — Add member hide and unhide controls after API support (blocked by t828, Task 3; after Task 15 so controls integrate with compact Member RowActions) — branch `dcjx-member-hide-controls`
- [ ] Task 19: `5z54` — Allow changing account type in the account edit UI (blocked by dgkf, Task 4; small frontend form change) — branch `5z54-account-type-edit-ui`
