...
- [ ] Task 8: `f6qg` — Add page-aware keyboard shortcuts help (P2 feature; introduces the page-aware shortcut catalog, the `?` modal, and its command-palette action, so it lands before Task 9 which registers additional palette behavior; subplan covers the catalog contract per route, the accessible modal with focus restore, the palette action, `docs/webui-design.md` shortcut-list updates, and browser coverage) — branch `f6qg-shortcuts-help-modal`
