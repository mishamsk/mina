# Plan: Clear the open P1/P2 Kata backlog — sequential sub-branch delivery (`kata ready` filtered to priority 1 and 2)

Deliver every open, unblocked priority-1 and priority-2 Kata issue as of 2026-09-13: one desktop fixed-shell scroll regression, the recurring-definition panel route leak, three transaction-entry draft bugs, a Categories toolbar alignment bug, settlement dates in account registers, page-aware keyboard shortcuts help, and modifier-aware account actions in the command palette. All nine issues are frontend work under `frontend/` with browser coverage; none has an open blocker. Deliver one Kata issue at a time as a Codex-implemented sub-branch of the main working branch, with the session running this plan acting as operator: plan author, reviewer, integrator. This plan is self-contained; it deliberately inlines a modified (strictly sequential) version of the codex-goal-fleet workflow and does not depend on that skill.
...
- Issue set: `kata ready --agent` on 2026-09-13, keeping only `priority=1` and `priority=2` issues: `k2gk`, `dhg3`, `me5f`, `nwv6`, `7gs0`, `kp0d`, `b2p2`, `r1yn`, `f6qg`.
...
Task scope details live in the Kata issues (`kata show <ref> --agent`). Respect the stated dependencies; otherwise run in the listed order. No task in this fleet has an explicit Kata blocker; the order below puts the only P1 first, then app-shell layout and route infrastructure, then the transaction-entry bug cluster (which shares draft-state code, so sequential delivery avoids merge conflicts), then isolated display fixes, then command-palette and shortcuts features with the shortcut catalog landing before its palette consumer.

- [x] Task 1: `k2gk` — Fix fixed-shell desktop canvas-scroll regressions (P1 bug; establishes the desktop fixed-shell layout contract across every route, so it lands before any other page work; subplan must cover Recurring plus the Categories/Tags window-overflow audit findings, classify document-scrolling report pages, extend the Recurring demo fixture or test data so the inner table actually overflows, and add representative browser coverage; related prior work `efrg` is closed and not a blocker) — branch `k2gk-fixed-shell-canvas-scroll`
- [x] Task 2: `dhg3` — Close recurring-definition panel when navigating away (P2; app-shell route/panel lifecycle change that rewrites the "route-independent recurring-definition editor" contract in `docs/webui-design.md` and the recurring feature package docs; subplan covers backlink pre-open, app navigation, in-app links, browser Back/Forward, dirty-draft protection, and browser coverage; independent of Task 1 but ordered next as shell infrastructure) — branch `dhg3-close-recurring-panel-on-navigation`
- [x] Task 3: `kp0d` — Fix keyboard behavior in entry-draft discard confirmation (P2 bug; first of the transaction-entry draft cluster because it is the smallest and isolates the confirmation dialog's focus trap; subplan covers initial focus on Discard, Tab/Shift+Tab traversal, Enter-to-discard, Escape-to-keep, and browser coverage) — branch `kp0d-discard-confirmation-keyboard`
- [x] Task 4: `nwv6` — Keep the active transaction-entry tab when clearing a draft (P2 bug; follows Task 3 because both touch entry-draft clearing and confirmation flow; subplan covers every create tab including Advanced, template selection reset, persisted-draft reset, and browser coverage for a non-Spend tab and Advanced) — branch `nwv6-keep-entry-tab-on-clear`
- [x] Task 5: `7gs0` — Seed new transaction dates from the active Transactions day (P2 bug; last of the entry cluster because launch-context state interacts with the draft baselines touched in Task 4; subplan covers Transactions-list launch seeding, no-default from every other entry point, transient non-persisted launch state, and browser coverage for a non-today day and a non-Transactions entry point) — branch `7gs0-seed-entry-date-from-active-day`
- [x] Task 6: `b2p2` — Show settlement dates in account registers (P2 feature; isolated register-row display change consuming the pending/posted timestamps the REST record shape already carries, so no API change; subplan covers posted-date and pending-date rows, browser-local presentation rules, an explicit tested policy for date-free records, and frontend coverage for both settlement states) — branch `b2p2-register-settlement-dates`
- [x] Task 7: `me5f` — Align the Economic intent selector with adjacent controls (P2 bug; isolated Categories toolbar polish with no dependency, ordered after the larger fixes; subplan covers outer height and vertical alignment against adjacent toolbar controls, preserved selection/keyboard/focus/responsive behavior, and focused geometry coverage) — branch `me5f-align-economic-intent-selector`
- [x] Task 8: `f6qg` — Add page-aware keyboard shortcuts help (P2 feature; introduces the page-aware shortcut catalog, the `?` modal, and its command-palette action, so it lands before Task 9 which registers additional palette behavior; subplan covers the catalog contract per route, the accessible modal with focus restore, the palette action, `docs/webui-design.md` shortcut-list updates, and browser coverage) — branch `f6qg-shortcuts-help-modal`
- [x] Task 9: `r1yn` — Add modifier-aware account actions to the command palette (P2 feature; after Task 8 so the modifier action is described in the shortcuts help catalog; subplan covers the Alfred-style subtitle on the active account result, Enter opening the register, platform-modifier Enter opening Transactions pre-filtered to the account, live modifier updates during result navigation, keyboard accessibility, and browser coverage) — branch `r1yn-palette-account-modifier-actions`
