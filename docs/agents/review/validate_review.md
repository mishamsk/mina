...
Decide whether the candidate is a real defect introduced by the exact review range. Success means the decision is supported by direct code inspection or a focused smoke test and only a confirmed finding can reach the fixer.
...
<candidate_finding> {{FINDING}} </candidate_finding>
