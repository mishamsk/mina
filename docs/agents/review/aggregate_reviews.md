...
Use `major`, `minor`, and `nit` with the meanings supplied by the reviewers. Return only blocks in one of these exact shapes, with actionable candidates first:
...
<raw_reviews> {{RAW_REVIEWS}} </raw_reviews>
