Review backend `app-tests` and launched-process `e2e-tests` only. First read and internalize `docs/TESTING.md` and `docs/BACKEND-TESTING.md`.
...
- New or changed tests violate `docs/BACKEND-TESTING.md`, especially by coupling `app-tests` to SQL, database schema, stores, services, routers, handlers, or private helpers.
