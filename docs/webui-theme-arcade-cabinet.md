...
- Expanded transaction rows latch the row-hover fill, drop their bottom hairline to fuse with the records subtable (closed by the subtable's 2px `--border-ink` bottom border), and step the title to mono SemiBold; the latched fill is flat state — no outline, shadow, or press feedback — and must read against both banding parities.
- In-table markers: in transaction lines, category, tags, and member render as square chips (ink outline + `--shadow-chip`); amounts render as chips too — white with ink text for all classes; tag chips drop to the micro size on a single ellipsis-truncated line. Records subtables use a bare role-glyph indicator in a narrow leading column. In the transaction detail and peek panels, category and tags render as standard entity chips (ink outline + `--shadow-chip`, never clipped) while the other columns and the per-record disclosure sub-row render plain undecorated table text (no chips, badges, or shadows). The inline expanded records subtable in the transactions list renders plain undecorated table text for every column except the role indicator.
...
- Lifecycle strip (transaction detail/peek header): a one-line `--band` fill row with 1px `--hairline` top and bottom; indicator class — no press affordance, no chip shadow. Each segment pairs an uppercase mono micro-label in `--muted-foreground` with its lifecycle-stage glyph (calendar, clock, or check) in the stage color and the date in ink; connectors are `--muted-foreground` chevron glyphs. The strip is a timeline, so every reached stage shows its glyph — posted included, the one exception to posted showing no marker in tables; unreached stages render a muted dash with no glyph.
- Affordance classes (per webui-design rules) map to: indicators = bare glyphs/badges with no press or hover affordance; entity chips = outlined chips with `--shadow-chip` plus an instant hover step (they filter the current view); actions = the trailing `RowActions` cluster. Button-class row actions are compact square icon buttons, always visible: outline-only at rest — never `--shadow-chip` at rest, so a row action can never read as a chip — gaining the chip shadow on hover and pressing in on activation. Disabled buttons (row actions and all arcade buttons) read unmistakably disabled: `--muted-foreground` outline and glyph on a muted fill, `not-allowed` cursor, an explanatory tooltip, and NO hover or press feedback — a disabled control never translates, never drops or gains shadow. The narrow-screen overflow (⋯) control uses the same icon-button treatment. Flat toggle icons are persistent bare glyphs with no frame: `--muted-foreground` when off, the relevant accent when on (yellow-filled star for featured, rendered unclipped; ink eye-off for hidden), with hover ink and tooltips — placement in the trailing column plus hover/press feedback keeps them from reading as indicators. Toggle/indicator glyphs sit in fixed per-column slots — star always in the slot left of the hidden eye, slots reserved even when a table has no featured flag yet — so the same glyph aligns vertically across all rows and reference tables.
- Toolbar state toggles (Include hidden, Hide expected) are standard outline buttons that latch the sky table-header tint while pressed (`aria-pressed`), swapping to an off/struck glyph variant; Hide expected uses `calendar-weeks` and a project-local `calendar-weeks-off` built with the set's eye-off stepped-strike technique.
...
- `FqnPath`: de-emphasized ancestors + emphasized leaf in mono; dense-cell leaf-chip variant per webui-design display rules. Link mode (detail/peek record account cells): rest state identical to the non-interactive path; hover underlines the whole anchor with each segment in its own text color and no fill (fill stays filter vocabulary); no press-in; standard focus ring, with collapsed middle segments expanding in place while focus is visible.
- `EntityPicker` popup: breadcrumb header is a sticky `--band` row with a 1px `--hairline` bottom — mono, ancestor crumbs in `--muted-foreground`, last crumb ink SemiBold, root glyph and chevron separators as pixel glyphs; group rows show the segment name with a trailing `chevron-right` pixel glyph and a de-emphasized mono child count in `--muted-foreground`; the create row uses a yellow-ink `+` glyph on an otherwise standard row; re-rooting appears in a single step per the sprite-motion rule — no easing.
