...
- In-table markers: in browse-mode transaction lines, category, tags, and member render as square chips (ink outline + `--shadow-chip`); amounts render as white chips with ink text for all classes; tag chips drop to the micro size on a single ellipsis-truncated line. Read-only markers keep their chip shadows without hover or press feedback. In Edit mode, mechanically eligible amount chips become white square-cornered inputs with class-accent ink, a visible currency marker, and the standard focus ring; ineligible amounts remain chips. Transaction detail record tables use a bare role-glyph indicator, standard category entity chips, and otherwise plain undecorated text; register peek category, tags, and member retain standard entity chips. Entity-chip shadows are never clipped.
