...
- Affordance classes (per webui-design rules) map to: indicators = bare glyphs/badges with no press or hover affordance; entity chips = outlined chips with `--shadow-chip` plus an instant hover step (they filter the current view); actions = the trailing `RowActions` cluster. Button-class row actions are compact square icon buttons, revealed on row hover/focus: outline-only at rest — never `--shadow-chip` at rest, so a row action can never read as a chip — gaining the chip shadow on hover and pressing in on activation; disabled ones use `--muted-foreground` outline and glyph with an explanatory tooltip. The narrow-screen overflow (⋯) control uses the same icon-button treatment. Flat toggle icons are persistent bare glyphs with no frame: `--muted-foreground` when off, the relevant accent when on (yellow-filled star for featured, ink eye-off for hidden), with hover ink and tooltips — placement in the trailing column plus hover/press feedback keeps them from reading as indicators.
