...
- Edit-mode tables: selected rows gain a sky/interactivity tint while the checked square checkbox carries the selection meaning; unavailable rows keep their normal content opacity. Selection checkboxes use the standard ink outline and chip shadow, then translate by the chip-shadow offset and drop the shadow when pressed. The persistent right-side control panel is an ink-bordered, square-cornered in-layout data surface with internal scrolling; its active editor uses ordinary form depth.
