...
- In-table markers: in transaction lines, category, tags, and member render as square chips (ink outline + `--shadow-chip`); amounts render as chips too — white with ink text for all classes; tag chips drop to the micro size on a single ellipsis-truncated line. Records subtables use a bare role-glyph indicator in a narrow leading column. In transaction detail, category renders as a standard entity chip while the per-record disclosure is plain undecorated text; in the register peek, category, tags, and member retain standard entity chips. These record-table entity-chip shadows are never clipped. Other record-table columns render plain undecorated text (no chips, badges, or shadows). The inline expanded records subtable in the transactions list renders plain undecorated table text for every column except the role indicator.
...
- Lifecycle strip (transaction detail/peek header): a one-line `--band` fill row with 1px `--hairline` top and bottom; uppercase muted mono `Initiated`, the civil date in ink, then a muted lowercase non-posted status word. No glyphs, connectors, shadows, or press affordance.
