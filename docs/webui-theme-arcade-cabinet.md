...
- Affordance classes (per webui-design rules) map to: indicators = bare glyphs/badges with no press or hover affordance; entity chips = outlined chips with `--shadow-chip` plus an instant hover step (they are links); actions = the trailing `RowActions` cluster. Button-class row actions use the standard button treatment at a compact in-table size (outline, chip shadow, press-in); disabled buttons drop the shadow and use `--muted-foreground` with an explanatory tooltip. Flat toggle icons are bare glyphs with no frame: `--muted-foreground` when off, the relevant accent when on (yellow-filled star for featured, ink eye-off for hidden), with hover ink and tooltips — placement in the trailing column plus hover/press feedback keeps them from reading as indicators.
