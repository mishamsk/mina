...
- Roomy shells use fixed left sidebar navigation, collapsible to an icon rail. Compact shells remove the sidebar from layout and expose the same navigation in an overlay opened from the bottom app toolbar. Sections: Overview, Transactions, Recurring, Accounts, then a Reference group (Categories, Tags, Members, Templates), then Status/Settings pinned at the bottom.
- A compact balance strip of featured accounts is visible from every screen within the active navigation surface. Entries use account display labels with full-FQN tooltips and link to account pages; featured is a backend account metadata flag in portable state.
...
- Authenticated shells expose a global Log out action at the bottom of the active navigation surface. Logout replaces the shell with login when authentication remains enabled; if authentication becomes disabled, the shell stays visible without the logout action. Any protected-request `401` replaces the shell with login.
...
- FQNs render as a segmented path in hierarchy views and choice controls that use the FQN as their primary label: ancestor segments de-emphasized, leaf segment emphasized, e.g. `banks:Chase:` (de-emphasized) `Joint` (emphasized). Title-first picker leaves follow the picker-specific rule below.
...
- Account trees remain grouped, searched, and sorted by FQN; picker leaves prefer the effective display title, reveal the complete FQN in a tooltip or while Meta is held, and retain complete FQNs in drilled-in breadcrumbs, while filters, restructure controls, and navigation autocomplete retain their owning identity rules.
...
- Row composition: class icon, initiated date, description (the `From → To` line) with the memo as a truncated second line (full memo in a tooltip) and trailing status indicators, category, tags, member, display amount, and the trailing explicit transaction-actions column. The description column header reads "Description". At phone widths, each row becomes two tiers: browse mode keeps class/date/status/amount/actions on top, Edit mode replaces actions with the selection checkbox, and both place account description and memo on one full-width line below; category, tags, and member collapse from the summary but remain in detail.
...
- In the roomy shell, the browser fills the available viewport height: the table body flexes and the pagination footer sits at a small, consistent inset from the viewport bottom, matching the sidebar's bottom-control inset so the two bottom edges align.
- The compact shell applies below `sm` and to 48–75rem-wide viewports no taller than 53rem, covering short tablet landscapes. Full-page table screens grow in normal document flow and use window scrolling with non-sticky headers. A fixed full-width bottom app toolbar provides equal Navigation and Controls actions plus a conditional Edit action while transaction Edit mode is active. Each action opens its existing content in a viewport-bounded full-width bottom sheet without reserving page width; compact picker and menu sheets temporarily replace an open parent sheet, then reveal it again on Back, selection, or dismissal. Open toolbar actions use a fill highlight without press translation. Escape, outside interaction, and focus restoration retain their existing ordering. Report previews retain their route-level scrolling and read-only controls while using the shared phone transaction-row geometry; overlay-owned tables remain unchanged.
...
- Entering swaps the toolbar for a compact `EDIT MODE` header with the live selection count, Select page, Clear, and Done; closes detail; reveals the leading checkbox column; places a persistent right-side control panel beside the table and pagination in the roomy shell; and adds Edit to the compact app toolbar.
- The control panel is available from zero selected and provides Account Replace, Category Replace, Tags Add/Remove, Member Set/Clear, and grouped settlement/reconciliation actions. It expands at most one labeled editor at a time and keeps mutation errors with that editor without losing its draft or selection. In the roomy shell, it stays visible and scrolls internally within the browser's fixed viewport height without moving below the table or displacing pagination; in the compact shell, it stays hidden until Edit opens it as a viewport-bounded bottom sheet.
...
- Account picker leaves show the effective display title and account type plus `CODE · Single-currency` for a non-`NULL` currency or `Multi-currency` for `NULL`; the complete FQN follows the shared tooltip, Meta-key, and breadcrumb rules. Selecting a single-currency account adopts its currency, while selecting a multi-currency account preserves the record currency; the amount stays unchanged.
...
- Edit mode provides selection-based quick changes through its in-layout dock in roomy shells or Edit bottom sheet in compact shells, plus selection-independent eligible amount inputs.
