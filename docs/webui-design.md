...
- Toolbar: search, go-to-day with icon-only day-step buttons, a sort menu with field and order controls, class dropdown, page-local native/USD amount toggle, and the Filter toggle; typed filter chips accumulate in the filter bar beneath the toolbar row.
...
- Categories: the standing toolbar places a compact Economic intent selector (`All`, `Income`, `Expense`) alongside search and include-hidden; selecting Income or Expense stores the typed `economic_intent` filter in the URL, while All omits it. Rows carry an economic-intent badge (`expense` or `income`); the editor requires intent and explains its classification effect in one line.
