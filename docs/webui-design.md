...
- Entity pickers are type-ahead comboboxes over entity-owned backend results with segment-by-segment completion. Each typed API context owns semantic eligibility, hidden and selected policy, navigation-group derivation, creation eligibility, matching, deterministic ordering, and the 20-row bound; the shared frontend owns query and hierarchy interaction, discards stale responses, preserves input through loading or retryable errors, and renders returned order unchanged. Groups are navigation only and committed values are always leaves.
...
- Eligible entry pickers keep entry unblocked with an inline "Create …": shorthand tabs create categories and flow accounts in merchant/counterparty fields, while tag pickers also create tags; Advanced selects existing categories and accounts. Each shorthand tab supplies its sole valid intent context — Spend and Refund use `expense`, Income uses `income` — so intent is never a question at entry. The entity API's `can_create` decision gates the final sticky action for the complete typed path; the mutation remains the validation authority. Drilling before creating namespaces the new leaf under the committed prefix.
