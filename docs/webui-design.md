...
- Account pickers filter intelligently by context: only account types valid for the field being edited (e.g. funding → `balance`, merchant → `flow`), derived from the intent shape rules in `docs/accounting-semantics.md`. This is deterministic filtering, never record-role guessing; invalid subtrees are absent from the popup, not disabled.
