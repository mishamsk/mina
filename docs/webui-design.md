...
- Transaction detail (URL-addressable, side panel over the list): class badge, counterparty title, display amounts, lifecycle strip, full record table, metadata (source, created). The panel is read-only: category, tags, member, amount, account, and record values are display-only, and no pointer or keyboard path starts an inline editor inside the panel; entity chips keep their filter behavior, and record-table account paths link to the account's register page per the hierarchical-names rule. Actions: a labeled Edit button in the panel header — the panel's primary action — plus Duplicate, Split, and Delete in the footer bar. Edit, Duplicate, and Split open the transaction editor modal over the panel; the panel stays open beneath it and refreshes after save. The detail view shows everything the summary line truncates or hides: complete tag sets, full memos, and all per-record values. Per-record dates and statuses follow the lifted-lifecycle model, identical in the detail panel and the account-register peek. A lifecycle strip directly under the panel header states the transaction's chronology once — Initiated, Pending, Posted in order, each stage showing its uniform day-level date, an explicit dash when unreached, and a min–max range with a literal "varies" marker (plus an "n of m" count when only some records reached the stage) when records disagree; expected occurrences render the first stage as the expected scheduled date; the full Initiated/Expected civil date and exact Pending/Posted timestamps sit in stage tooltips. The record table has no dates column; a record whose dates or posting status deviate from the strip shows its deviating day-level values inline in its status cell, as text with the status glyph, never by glyph or color alone. Row activation (click, Enter, or Space; `aria-expanded`) toggles a read-only per-record disclosure listing the full Initiated/Expected civil date, exact Pending/Posted timestamps, posting status, role, source, and the untruncated memo — every per-record value is at most one row activation away, tooltips only ever supplement that path, and the disclosure never edits. The strip is financial chronology only; created stays in metadata.
...
- Type tabs: Spend, Income, Refund, Transfer, Exchange, Advanced. Shorthand forms render in a centered narrow column; Advanced fills the body width; the frame never changes between tabs.
  ...
  - Exchange: date, from account with the sold amount, to account with the bought amount, optional tags/member/memo. Two amounts in two currencies, with the effective rate as read-only feedback; Mina supplies the `system:exchange` records, so no counterparty is ever picked.
...
- Account page header: FQN path, account type badge, currency, labeled flat favorite toggle, current balance and posted-only balance, credit limit with history (when present), external link metadata, hidden marker. Fixed system accounts replace mutation controls with a read-only indicator.
...
- Layout: tree table grouped by FQN hierarchy; columns: name (path-indented), type badge, currency (ISO code), balance (`owned` and `party` accounts), and the trailing actions column. Hidden state renders as the standard eye-off indicator on the row, not as its own wide column. Rows link to account/group pages.
