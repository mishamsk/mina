...
- Implemented API capability groups:
  ...
  - Account, category, tag, and household member CRUD/list flows, including account type changes, account, category, tag, and member delete eligibility, member hidden-state updates and include-hidden listing, account featured metadata, and category economic-intent filtering.
...
- Implemented web UI behavior:
  ...
  - Transactions page uses server-derived transaction titles, a standing URL-backed transaction-class toolbar filter, URL-backed server-side search/filter chips, date-jump anchored pagination with adjacent-day controls, trailing hover-revealed open-detail and quick-delete row actions, entity chips that add filters in place, and a URL-addressable detail side panel with full journal records and delete confirmation.
  - Recurring review screen surfaces EXPECTED occurrences with overdue emphasis, confirm/dismiss actions, and an explicit Expected posting-status transaction filter path.
