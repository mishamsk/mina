...
- Transactions support record-currency and other record/transaction filters, pagination, date anchoring, free-text search, and stable initiated/created/updated ordering; ETag-guarded identity-preserving full replacement with protected importer provenance and dependent records; independent derived class, shape, role, and display metadata; dry-run classification; and record-level bulk category, tag, member, account, settlement, and reconciliation updates.
...
- The REST API is OpenAPI-defined, exposes the implemented accounting, reference, template, recurring, rates, operation, settings, demo, health, and authentication contracts, and serves its generated spec at `/api/openapi.json`. Applicable timestamped collections support stable ascending and descending updated-date ordering. Transport validation and stable machine-readable JSON errors are in place.
- Every matched non-`GET` OpenAPI request is recorded best-effort in portable audit history with structured outcome metadata, caller-declared surface attribution, and eligible JSON payloads; login passwords are removed before persistence, bounded newest-first audit reads are available through REST, CLI, and MCP, and expired history can be compacted manually or monthly under ordinary retention settings.
...
- REST, generated MCP, and `mina db schema` expose the embedded current target accounting DDL for inspection; it is not the configured database's live schema or a database initialization path.
- `mina client --server URL` is a remote JSON CLI with optional `MINA_API_KEY`. `mina client --db PATH` runs the same selected REST operations against a one-shot in-process app: it has no listener, authentication, startup validation, or automatic operations, but waits for manually triggered exchange-rate, backup, and audit-compaction runs to finish.
...
- The browser provides an overview with configurable household flow, active owned/party balances, and monthly activity; a persistent featured-account strip; searchable account, category, tag, and member management with URL-backed category intent filtering; account/group registers; configurable Category/Tag flow overviews; member transaction drill-downs; a Settings screen; and a Status screen for health, database encryption and file size, plus URL-addressable background-operation and API audit-history browsers. Account display-label overrides affect presentation only; hierarchy, navigation, and disambiguation remain FQN-based.
- Transaction browsing and account/group registers share one URL-addressable full transaction detail with native/USD display, lifecycle and settlement indicators, transaction and expanded-record update metadata, and the same actions. Transaction browsing focuses changing detail, while registers retain record-row focus for arrow-key browsing. Transaction browsing adds filters, configurable stable ordering, date navigation for the default order, and selection-based reference/status edits; transaction entry is a route-independent modal for spend, income, refund, transfer, exchange, and advanced balanced entries, including pending creation, compatible template defaults, draft clearing, edit/duplicate, split, post, and cancel flows.
...
- Exchange-rate loading starts from the configured Frankfurter cache and supports scheduled and manual targeted refreshes. Database backup runs manually, and on a schedule when configured; they copy only file-backed accounting databases to configured local targets and preserve database encryption. API audit-log compaction runs manually or on its configured UTC schedule, monthly by default, and uses a calendar-month retention boundary.
