...
- Implemented web UI behavior:
  ...
  - Transactions page uses server-derived transaction titles, a standing URL-backed transaction-class toolbar filter, URL-backed server-side search/filter chips, date-jump anchored pagination with adjacent-day controls, trailing hover-revealed row actions, entity chips that add filters in place, and a URL-addressable detail side panel with full journal records and delete confirmation.
