...
- Implemented web UI behavior:
  ...
  - Transactions page uses server-derived transaction titles, a standing URL-backed transaction-class toolbar filter, URL-backed server-side search/filter chips, date-jump anchored pagination with adjacent-day controls, trailing hover-revealed open-detail and quick-delete row actions, entity chips that add filters in place, URL-addressable detail, uniform transaction-row/detail-panel category/tags/member edits, simple two-record amount edits, and per-record inline category, tags, member, memo, date, and posting-status editing in expanded and detail-panel journal records.
