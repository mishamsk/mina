...
SELECT
	(SELECT MIN(record_id) FROM demo.journal_record WHERE tombstoned_at IS NULL),
	(SELECT MIN(record_id) FROM demo.journal_record WHERE tombstoned_at IS NOT NULL),
	'REFUND';
