...
// SearchContext identifies a tag search's domain eligibility rules.
...
// SearchOptions contains typed tag discovery inputs.
...
// SearchItemKind distinguishes tag leaves from navigation groups.
...
// SearchItem is one display-ready ranked tag candidate.
...
// SearchResult is a bounded ranked tag candidate response.
type SearchResult struct {
	Items   []SearchItem
	HasMore bool
}

// Search returns caller-bounded ranked tag leaves and groups for one typed context.
func (s *Service) Search(ctx context.Context, opts SearchOptions) (SearchResult, error) {
	...
	for _, state := range states {
		...
		if !state.active || (!opts.IncludeHidden && state.reference.IsHidden && !exactFQNQuery) {
			continue
		}
	...
	for _, leaf := range leaves {
		...
		item := SearchItem{Kind: SearchItemKindLeaf, ID: &id, Title: title, FQN: leaf.FQN, IsHidden: leaf.IsHidden}
...
func validateSearchOptions(opts SearchOptions) error {
	...
	default:
		return services.InvalidRequest("invalid tag search context")
	}
