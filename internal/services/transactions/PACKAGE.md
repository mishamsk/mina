...
- Transaction lists validate record-currency filters through the shared currency-code contract and reject missing or inactive dictionary filters rather than treating them as empty results. Account-scoped record search maps an invalid target account to not found, and only it may request running balances.
