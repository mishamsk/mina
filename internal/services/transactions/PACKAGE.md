...
- Transaction lists validate record-currency filters through the shared currency-code contract; transaction lists and record searches reject missing or inactive dictionary filters rather than treating them as empty results. Transaction and record lists support updated-date ordering with their own identities as stable tiebreakers. A transaction update timestamp advances for transaction metadata, lifecycle, or nested-record changes. Account-scoped search maps an invalid target account to not found, and only it may request running balances.
