...
type categoryReferenceCache struct {
	mu      sync.RWMutex
	loaded  bool
	entries map[int64]categoryReferenceState
}
...
func (s *Service) ensureReferenceCache(ctx context.Context) error {
	...
	for _, category := range categories {
		entries[category.ID] = categoryReferenceState{
			reference: Reference{
				ID:             category.ID,
...
func (s *Service) cacheActiveReference(category Category) {
	...
	if s.cache.entries == nil {
		s.cache.entries = map[int64]categoryReferenceState{}
	}
	s.cache.entries[category.ID] = categoryReferenceState{
		reference: Reference{
			ID:             category.ID,
			EconomicIntent: category.EconomicIntent,
			IsHidden:       category.IsHidden,
