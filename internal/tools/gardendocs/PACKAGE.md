...
- The last sentinel commit touching a target excludes it from selection. `--limit` applies only to package documents; `PROJECT_STATE.md` is selected separately.
...
- Owns: target classification, gardening eligibility, generated dependency context, agent concurrency, and per-target Git commits.
