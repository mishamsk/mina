...
3. Add the smallest data set that distinguishes preservation and the migration-specific transformation. Prefer `env -u MINA_DATABASE_ENCRYPTION_KEY -u MINA_SCHEMA ./bin/mina --config-file /dev/null client --db /tmp/mina-vNNNNN.duckdb --yes ...`; use direct DuckDB data changes only for legacy states the public surface cannot express, and never hand-author schema objects.
