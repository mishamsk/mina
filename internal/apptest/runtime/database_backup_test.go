...
func TestDatabaseBackupOperationExpectedBehavior(t *testing.T) {
	...
	t.Run("run transitions through operation-run status fields", func(t *testing.T) {
		...
		if string(run.OperationId) != "database-backup" ||
			...
			run.Outcome != httpclient.BackgroundOperationRunOutcomeSucceeded ||
			run.CompletedAt == nil ||
			run.Error != nil {
	...
	t.Run("concurrent manual triggers produce one running run and one skipped run", func(t *testing.T) {
		...
		if running.JSON200.Outcome != httpclient.BackgroundOperationRunOutcomeRunning || running.JSON200.CompletedAt != nil {
	...
	t.Run("in-memory accounting backup fails with stable operation error", func(t *testing.T) {
		...
		run := client.PollDatabaseBackupRun(started.JSON202.OperationRunId)
		if run.Outcome != httpclient.BackgroundOperationRunOutcomeFailed || run.Error == nil || *run.Error != "in-memory accounting database cannot be backed up" {
	...
	t.Run("non-empty schedule creates recurring runs under fake clock", func(t *testing.T) {
		...
		clock.WaitForPendingDeadlineWaits(t, 3)
		if calls := clock.DeadlineWaitCalls(); calls != 3 {
			t.Fatalf("shared deadline wait calls = %d, want backup, audit-compaction, and recurring-catch-up waits", calls)
		}
		...
		clock.WaitForPendingDeadlineWaits(t, 3)
		if calls := clock.DeadlineWaitCalls(); calls != 4 {
