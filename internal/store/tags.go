...
// NewTagStore creates a tag store using AppDB.
func NewTagStore(db *AppDB) *TagStore {
...
WHERE tag_id = ?`
	...
	tag, err := scanTag(s.db.query().QueryRowContext(ctx, query, args...))
...
// SetHiddenByPath sets hidden state on active tags at or below path.
func (s *TagStore) SetHiddenByPath(ctx context.Context, path string, hidden bool) error {
...
WHERE tombstoned_at IS NULL
  AND (fqn = ? OR starts_with(fqn, ? || ':'))`,
	...
	if err != nil {
		return fmt.Errorf("set tag hidden by path: %w", err)
	}
