...
- Account balances, and account-register records when running balances are requested, load current credit limits separately to derive `remaining_credit` from the reported balance. This balance presentation mapping must not change service balance semantics. Record-search mapping preserves service-supplied transaction display titles without deriving transaction semantics in transport.
...
- Account, Category, Tag, and Member search operations map typed contexts, caller limits, exclusions, bounded entity-shaped rows, and `has_more` without transport-side eligibility, hierarchy derivation, or scoring. Account, Category, and Tag creation-availability reads map stable advisory reasons; only Account exposes `reserved_namespace`.
- Account, Category, Tag, and Member lists forward fuzzy membership and typed filters without transport-side matching; Account type is repeated so one or several values use the same contract.
