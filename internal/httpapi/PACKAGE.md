...
- Account, Category, Tag, and Member picker operations map typed query contexts, bounded entity-shaped rows, and independently returned selected rows without transport-side eligibility, hierarchy derivation, or scoring; Account picker mapping also carries the service-owned complete eligible count. They remain `GET` Web UI support operations excluded from CLI and MCP catalogs.
